"""Shared test helpers that are safe to import from test modules."""
...
def make_spot_json(name: str, slug: str, start: dt.datetime, hours: int, *,
    ...
    for i, model_name in enumerate(model_names):
        models[model_name] = {"hourly": {
            "time": axis,
            "wind_speed_10m": [wind + i * 1.0] * hours,
    ...
    marine_models = {}
    if wave_models:
        for model_name, (mhs, mtp) in wave_models.items():
            marine_models[model_name] = {"hourly": {
                "time": axis,
